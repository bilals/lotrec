...
3. **Developer experience**: Building and modifying Swing UIs involves verbose boilerplate code. JavaFX offers declarative layout (FXML), property binding, and CSS, significantly reducing development effort for future features.
...
4. **UC-4: Developer maintains or extends the GUI** — A developer modifies or adds GUI components using modern JavaFX patterns (CSS styling, FXML layouts, property bindings) rather than legacy Swing boilerplate, reducing development time for future enhancements.
...
| FR-09 | The screenshot capture mechanism shall cover all key application states: main frame, each tab, each dialog, tableau display, and control panel states | Should |
...
| FR-16 | The system shall display Cytoscape graph visualizations within the JavaFX interface using a compatibility bridge until a native replacement is available (Phase 4 of roadmap) | Must |
| FR-17 | The system shall provide a new JavaFX application entry point that can be launched independently of the Swing entry point | Must |
...
| FR-22 | The old Swing GUI shall remain functional and launchable via the existing entry point throughout the migration | Must |
...
|----|-------------|--------|
| NFR-01 | The application startup time shall not regress | New GUI displays main window within same timeframe as current Swing GUI |
...
- [ ] **JavaFX**: New GUI framework replacing Swing (constitution-approved migration path)
...
| FormulaTransformerDialog | `lotrec.guifx` | JavaFX formula transformer |
| Various dialogs | `lotrec.guifx.dialogs` | JavaFX equivalents of all Swing dialogs |
...
|------|------------|--------|------------|
| Cytoscape SwingNode bridge has rendering or threading issues | Medium | High | Isolate bridge in its own component; test early in migration order; accept minor visual compromises in bridge area |
...
4. **CSS styling targets structural equivalence**: The JavaFX CSS will replicate the current Swing layout structure and control placement, with functional improvements where modern UX best practices dictate. A full visual redesign is out of scope, but incremental UX improvements are in scope.
