 ...
 * @author said
 */
public class PriorityInfixToPrefix extends SLR1_Grammar {
    ...
    public static void main(String[] a) throws Exception {
        ...
        PriorityInfixToPrefix i2i = new PriorityInfixToPrefix(connList);
...
//        ParseTree pTree = (ParseTree) i2i.interpret(new String("pos nec imp or W C H"));
    ...
    private int outDecorator = PriorityInfixToPrefix.DEFAULT_DECORATOR;
    private ArrayList<Connector> connectors;

    public PriorityInfixToPrefix() {
        this.connectors = new ArrayList<Connector>();
    }

    public PriorityInfixToPrefix(ArrayList<Connector> connectors) {
        ...
        Semantics semantics = new Semantics() {
            ...
            public void f(ParseTree t, int l) {
                ...
                for (String outPart : copyOutParts) {
                    if (outPart.startsWith(PriorityInfixToPrefix.FORMULA)) {
                        String arg = outPart;
...
//            outputParts.add(PriorityInfixToPrefix.FORMULA+priorityLevel);
//            priorityLevel++;
//        }
        //We suppose that argsCount could not be 0!!!
        outputParts.add(PriorityInfixToPrefix.FORMULA + priorityLevel);
        priorityLevel++;
...
//                priorityLevel++;
//            }
            inputParts.add(PriorityInfixToPrefix.FORMULA + priorityLevel);
//            priorityLevel--;
    ...
    public void compile() throws Exception {
        ...
        this.setOutDecorator(PriorityInfixToPrefix.FIRST_LETTER_TO_UPPER_CASE_DECORATOR);
...
//        put("Constant", expression("[[:upper:]]?([[:alpha:]]|[[:digit:]])*"));
        ...
        Semantics constant = new Semantics() {
            ...
            @Override
            public void f(ParseTree t, int l) {
                if (outDecorator == PriorityInfixToPrefix.FIRST_LETTER_TO_LOWER_CASE_DECORATOR) {
                    t.value = firstLetterToLowerCase(t.child[l - 1].value.toString());
                } else if (outDecorator == PriorityInfixToPrefix.FIRST_LETTER_TO_UPPER_CASE_DECORATOR) {
        ...
        put(PriorityInfixToPrefix.FORMULA, new Object[][]{
                    {PriorityInfixToPrefix.FORMULA + 0, identity}
                });
        ...
        for (int i = 0; i < lastPriority; i++) {
            ...
            productions[iConnectors.size()] = new Object[]{
                        PriorityInfixToPrefix.FORMULA + (i + 1), identity
                    };
            put(PriorityInfixToPrefix.FORMULA + i, productions);
        }
        put(PriorityInfixToPrefix.FORMULA + lastPriority, new Object[][]{
                    {"Constant", constant},
                    {"(", PriorityInfixToPrefix.FORMULA, ")", parentheses}
                });
