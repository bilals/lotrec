...
public class RuleTransferHandler extends TransferHandler {
    ...
    public RuleTransferHandler(RulesTabPanel rulesTabPanel) {
    ...
    public boolean importData(TransferHandler.TransferSupport info) {
        ...
        try {
            rule = (Rule) info.getTransferable().getTransferData(RuleTransferable.ruleFlavor);
...
//                System.out.println("The rule " + rule.getName() + " should replace the rule " + logic.getRules().get(index) + " in the logic " + logic.getName() + " at position " + index);
    ...
    protected Transferable createTransferable(JComponent c) {
        JList list = (JList) c;
        Rule r = this.ruleTabPanel.getLogicDefTab().getLogic().getRules().get(list.getSelectedIndex());
        this.ruleTabPanel.setWasSourceOfDrag(true);
        return new RuleTransferable(r);
...
//        return TransferHandler.COPY;
    ...
    protected void exportDone(JComponent c, Transferable data, int action) {
        ...
        Rule rule = null;
        Logic logic = ruleTabPanel.getLogicDefTab().getLogic();
        RuleTransferable ruleTransferable = (RuleTransferable) data;
        try {
            rule = (Rule) ruleTransferable.getTransferData(RuleTransferable.ruleFlavor);
        } catch (UnsupportedFlavorException ufe) {
            System.out.println("importData: unsupported data flavor" + ufe.getMessage());
        } catch (IOException ioe) {
            System.out.println("importData: I/O exception" + ioe.getMessage());
...
//            System.out.println("Rule " + rule.getName() + " should be deleted form logic " + logic.getName() + " at index " + index);
    ...
    public boolean canImport(TransferHandler.TransferSupport support) {
        ...
        if (support.isDataFlavorSupported(RuleTransferable.ruleFlavor)) {
            return true;
        }
        return false;
    }
