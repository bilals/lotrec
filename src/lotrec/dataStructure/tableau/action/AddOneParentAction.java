 ...
 */
public class AddOneParentAction extends AbstractAction {
    ...
    public AddOneParentAction(SchemeVariable sourceNodeScheme, SchemeVariable nodeScheme, Expression relationScheme) {
        super();
    ...
    public Object apply(EventMachine em, Object modifier) {
        ...
        Edge edge = n.hasParent(e);
        if (edge != null) {
            TableauNode node = (TableauNode) edge.getBeginNode();
            return instanceSet.plus(nodeScheme, node);
        }

        TableauNode newNode = new TableauNode();//%%nodeScheme.toString());

        TableauEdge newEdge = new TableauEdge(newNode, n, e);
